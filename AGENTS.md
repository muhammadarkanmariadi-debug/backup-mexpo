...
| Frontend | `dev-frontend-mexpo-new/` | Next.js **16.2.6 (custom build — see ⚠ below)**, React 19, Tailwind CSS **v4 (CSS-first, no tailwind.config.ts)**, radix-ui + shadcn-style components, react-hook-form + zod v4, zustand (auth/global state), **TanStack Query v5 (`useApiQuery`/`useApiMutation`/`useList` — server data, see `docs/TANSTACK-QUERY.md`)**, server actions + fetch wrapper (`"use server"` http-client, httpOnly-token auth), lucide-react + fontawesome, framer-motion, leaflet |
...
> ⚠ **Frontend: "This is NOT the Next.js you know."** `dev-frontend-mexpo-new/AGENTS.md` states this Next.js 16 build has breaking changes. Read the relevant guide in `node_modules/next/dist/docs/` before writing any frontend code. Note `middleware` is renamed to **`proxy.ts`** and it **must live in `src/proxy.ts`** (the dir that contains `src/app`) — a root-level `proxy.ts` is silently ignored and produces an empty `middleware-manifest.json` (fixed 2026-08; see Blockers).
...
└─ dev-frontend-mexpo-new/
   ...
   └─ src/proxy.ts                 # ✓ route protection (Next 16 proxy) — reads `token` cookie, guards /dashboard/:path* + /profile
```
...
| `dev-frontend-mexpo-new/docs/TANSTACK-QUERY.md` | **Before adding any client-side data fetching** — how TanStack Query is wired (`useApiQuery` / `useApiMutation` / `useList` / `keys`), flow, and when NOT to use it (zustand, server components). |
...
- **Frontend structure:** feature folders under `src/features/<area>/`, shared UI under `src/shared/components/`. `src/components/ui/` holds only `pagination.tsx`. Data-fetching pages are **server components** using the `httpRequest` wrapper; interactive bits are `"use client"`; forms use `react-hook-form` + zod.
- **Frontend server data (TanStack Query v5):** client components read server data via `useApiQuery` / lists via `useList`, and mutate via `useApiMutation` (auto `invalidateQueries`). Query "addresses" live in `src/lib/query-keys.ts`. Do **not** add new `useEffect`+`useState` fetch blocks or call services directly in `useEffect` for server data — see `dev-frontend-mexpo-new/docs/TANSTACK-QUERY.md`. `zustand` stays for auth/global state only.
...
- **Design tokens:** use CSS variables/`@theme` tokens (brand `#3c85f3`, `gray-900 #101828`), never raw hex in components. Reuse shared components before adding new shadcn primitives. `teal`/`fuchsia` accent families are registered tokens.
- **Single button primitive:** use `src/shared/components/button/Button.tsx` (variants `primary/secondary/ghost/outline/danger/success`, sizes `xs/sm/md`). Use native `<select>` (no shadcn `ui/select`).
- **Dashboard page headers:** use `src/shared/components/ui/PageHeader.tsx` (`title/subtitle/icon/action/align`), not hand-rolled `<h1>` blocks.
- **QR flows:** check-in/booth/souvenir share `useQrScanner` + `QrScanPanel`; resolve a QR via `useResolveQr` (TanStack mutation). Do not add per-page scanner copy or imperative `resolveQr` calls.
- **Chart colors:** import from `src/shared/data/chart-colors.ts` (CSS-var tokens); never raw hex in recharts `fill`.
- **Dark mode:** `ThemeProvider` is mounted in the root layout (opt-in, light default) — keep `dark:*` classes; add toggles via `useTheme().toggleTheme`.
...
3. **Tickets / paid events — BUILT (Sprint 3/A1)** — `events.ticket_mode` (`FREE|PAID`), `ticket_types`, `tickets`, public purchase flow at `/event/[uuid]/register`, owner config at `/dashboard/[uuid]/registration`. **A1b (2026-08): Midtrans Snap gateway** — `src/payments/` module, `transactions` + `event_settlements` tables, checkout → Snap token, webhook `/payment/notification` (SHA512 + idempotent), escrow settlement by SUPERADMIN. Manual/POS fallback (free-form `payment_reference`/`payment_method` CASH/QRIS/TRANSFER) kept. Ticket emails not sent (A11).
   - **Dynamic registration form — BUILT (Sprint 3/A8)** — `event_registration_fields` (7 types) + `registration_answers`. Public schema via `GET /public-api/registration-fields/:event_id`; registration POSTs `answers[]` and the backend validates required fields. **Answers are readable by committee/owner since 2026-08** — `GET /event-users/:event_id` attaches `registrationAnswers[]` (with human labels) per user, rendered in the dashboard `VerificationPage`. The legacy hardcoded `users_bio` path in `public-api` is **still active** — do not remove without the A8 clarification decision.
...
9. **Frontend `proxy.ts`** — rewritten (FIX-07) to read `request.cookies`; do not import `"use server"` modules there. **Location matters:** in this Next 16 build the proxy MUST be at `src/proxy.ts` (same dir as `src/app`). A root `proxy.ts` compiles the module into the bundle but leaves `middleware-manifest.json` empty, so it never runs. Verified working 2026-08: `/dashboard` and `/profile` → 307 `/auth?next=...` when the `token` cookie is absent.
...
27. **Certificate templates (A10 upgrade) — BUILT** — `certificate_templates` table + `src/certificates/` module (GET/POST/PUT/DELETE `/certificates/templates/...`, active template endpoint). **Konva trap:** `react-konva@19.2.5` + `konva@^9` are canvas/window-bound. The shared renderer `src/features/certificates/CertificateStage.tsx` and the designer (`src/features/dashboard/certificate-designer/`) MUST be mounted via `next/dynamic(..., { ssr: false })` (see `CertificateDesignerWrapper.tsx` / `CertificatePreview.tsx`) — importing them directly into a server-rendered tree breaks SSR. Template `Text` nodes carry `attrs.binding: { type:'static', value } | { type:'dynamic', key }`; dynamic `key`s are **server-validated against a whitelist** in `certificates.service.ts`, so adding a new dynamic field means updating BOTH `CERTIFICATE_BINDING_KEYS` (backend) and `CERTIFICATE_FIELDS` (frontend `certificate-fields.ts`). Background images upload to S3 bucket `expo-project-certificate`. The visitor page falls back to the legacy hardcoded HTML certificate when an event has no active template.
28. **Payments (A1b — Midtrans Snap escrow) — BUILT.** New `src/payments/` module + `transactions`/`event_settlements` tables + `events.payout_*` columns. Key traps:
   - **Webhook** `/payment/notification` has **no guard by design** — it verifies SHA512 `sha512(order_id+status_code+gross_amount+ServerKey)` in `MidtransService`; never bypass or weaken that. Accept JSON **and** form-encoded bodies.
   - **Idempotency:** all status writes use `updateMany({ where: { uuid, status: 'PENDING' } })` so repeated webhooks can't clobber a final state. Keep that invariant in any new status logic.
   - **Amount source:** `amount`/`platform_fee` are Int IDR computed server-side from `ticket_types.price`; the notification handler also rejects a `gross_amount` mismatch. Do NOT trust client values.
   - `transactions.ticket_id` is intentionally **1:M** (re-checkouts after expiry create a new intent for the same ticket) — don't make it `@unique`.
   - Public (`/public-api/registration`) returns `data.payment` (Snap intent) for PAID events; failure to mint a Snap token is non-blocking.
   - `midtrans.service.ts` uses native `fetch` — no `midtrans-client` SDK dependency.
